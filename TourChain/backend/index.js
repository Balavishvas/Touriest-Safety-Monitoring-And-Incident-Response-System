...
// ANTIGRAVITY DEBUG PROBE
console.log("ANTIGRAVITY DEBUG: Server is initializing...");
...
// AI Engine Proxy for Demo
app.post("/api/ai/get-safety-score", async (req, res) => {
    try {
        const axios = require('axios');
        const AI_ENGINE_URL = process.env.AI_ENGINE_URL || 'http://localhost:5001';

        const response = await axios.post(`${AI_ENGINE_URL}/calculate-score/`, req.body);
    ...
    try {
        ...
        try {
            ...
            const panicCall = new PanicCall({
                userId: (panicData.userId && panicData.userId !== 'anonymous') ? panicData.userId : null,
                journeyId: panicData.activeJourney?._id || null,
                location: panicData.location || { lat: null, lng: null },
                type: panicData.type || 'Emergency',
                additionalData: {
                    userName: panicData.userName,
                    phone: panicData.phone,
