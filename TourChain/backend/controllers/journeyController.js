...
exports.startJourney = async (req, res) => {
    ...
    try {
        ...
        console.error('Start Journey Exception:', error.message);
...
exports.handleGpsCheckIn = async (req, res) => {
    ...
    try {
        // Update latest location in profile for easy access by admin
        ...
        // If no active journey, we just stop here but return success (profile updated)
        if (!activeJourney) {
            return res.status(200).json({ message: 'Profile location updated. No active journey to record history.' });
        }
