...
const generateEfirId = async () => {
    ...
    return `IR-${String(count + 1).padStart(5, '0')}`;
};
exports.createEfirFromPanic = async (panicCall) => {
    try {
        ...
        if (!profile) {
            console.error('❌ Incident Report Creation Failed: Tourist profile not found.');
            return;
        }
        ...
        console.log(`✅ INCIDENT REPORT FILED: ${efirReport.efirId} for Panic ID: ${panicId}`);

    } catch (error) {
        console.error('❌ CRITICAL ERROR during Incident Report creation:', error);
    }
};